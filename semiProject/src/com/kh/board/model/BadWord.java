...
public class BadWord {
	private String[] arr = {"심한 욕","심한욕","ㅅㅂ", "씨발", "씨바", "개세끼", "18년", "18놈", "18새끼", "ㄱㅐㅅㅐㄲl", "ㄱㅐㅈㅏ", "가슴만져", "가슴빨아", "가슴빨어", "가슴조물락", "가슴주물럭", "가슴쪼물딱", 
	        ...
	        "게새끼", "게늠", "게자식", "게지랄놈", "고환", "공지사항", "귀두", "깨쌔끼", "난자마셔", "난자먹어", "난자핧아", "내꺼빨아", "내꺼핧아", "내버지", "내자지", "내잠지", 
	...
	//게시글 욕설 필터링(제목,내용)
	public String[] badWord(String title,String content) {
		int count =0;
		for(int i=0; i<arr.length; i++) {
			if(title.contains(arr[i])) {//제목에 욕이 있다면
				for(int j=0; j<arr[i].length(); j++) {//욕 첫글자 포함 다 바꿈 
					title = title.replace(String.valueOf(arr[i].charAt(j)), "*");
				}
				count++; //count가 오르면 필터링에 걸렸다는 뜻
			}
			if(content.contains(arr[i])) {//내용에 욕이 있다면
				for(int j=0; j<arr[i].length(); j++) {//욕 첫글자 포함 다 바꿈 
					content = content.replace(String.valueOf(arr[i].charAt(j)), "*");
				}
				count++; //count가 오르면 필터링에 걸렸다는 뜻
			}
		}
		
		String[] arr2 = {title,content,Integer.toString(count)};
		
		return arr2;
		
	}
	//댓글 욕설 필터링(내용)
	public String[] badWord(String content) {
		int count =0;
		for(int i=0; i<arr.length; i++) {
			if(content.contains(arr[i])) {//내용에 욕이 있다면
				for(int j=0; j<arr[i].length(); j++) {//욕 첫글자 포함 다 바꿈 
		...
		String[] arr2 = {content,Integer.toString(count)};
